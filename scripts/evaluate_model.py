...
def save_results(args, row_name, dataset_name, logs_path, logs_prefix, dataset_results,
    ...
    iou_thrs = np.array([0.80, 0.85, 0.90, 0.95, 0.99]).tolist()
...
def get_prediction_vis_callback(logs_path, dataset_name, prob_thresh):
    ...
    def callback(image, gt_mask, pred_probs, sample_id, click_indx, clicks_list):
        ...
        image_with_mask, mask = draw_with_blend_and_clicks(image, pred_probs > prob_thresh, clicks_list=clicks_list)
        mask=np.repeat(mask[..., np.newaxis], 3, 2) * 255
        gt_mask = np.repeat(gt_mask[..., np.newaxis], 3, axis=2)*255
        cv2.imwrite(str(sample_path), np.concatenate((image_with_mask, gt_mask, mask, prob_map), axis=1)[:, :, ::-1])
