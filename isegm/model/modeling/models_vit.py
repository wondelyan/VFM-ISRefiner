...
class VisionTransformer(nn.Module):
    ...
    def forward_backbone(self, x, additional_features=None, shuffle=False, early_stage_output=True, early_feature_index=[2,3]):
        ...
        #         if early_stage_output and i == early_feature_index:
        #             early_features = x
        # else:
        ...
        #         if early_stage_output and i == early_feature_index:
        #             if is_patchified: 
        #                 x_unpatchified = self.unpatchify(x) 
        #                 early_features = x_unpatchified
        #             else:
        #                 early_features = x

        # if early_stage_output:
        ...
        intermediate_features = []  # 存储第2、3层特征
        ...
        # 对第2、3层特征进行加权融合
        # if len(intermediate_features) == len(early_feature_index):
        #     fused_features = 0.3 * intermediate_features[0] + 0.7 * intermediate_features[1]
        # else:
        #     fused_features = intermediate_features[0] if intermediate_features else x

        if early_stage_output:
            return x, intermediate_features  # 返回三个值: 最终特征、早期特征、融合特征
        return x  # 返回两个值: 最终特征、融合特征
