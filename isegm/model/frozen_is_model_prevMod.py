...
from isegm.model.modulation_new import modulate_prevMask
from isegm.model.modeling.refiner import Refiner
import math


class GatedFusion(nn.Module):
    """
    Used to fuse features from two different layers of the ViT backbone.
    """
    ...
    def forward(self, feat1, feat2):
        fusion_input = torch.cat([feat1, feat2], dim=-1)  # [B, N, 2C]
        gate = self.gate(fusion_input)                    # [B, N, C]
...
class ISModel_prevMod(nn.Module):
    def __init__(self, with_aux_output=False, norm_radius=5, use_disks=False, cpu_dist_maps=False,
        ...
        self.mask_token = nn.Parameter(torch.randn(1, 768) * 0.02)
        self.gatefusion=GatedFusion(768)
        self.click_refiner= Refiner(**refiner_params)

    def forward(self, image, points):
        image, prev_mask = self.prepare_input(image)
        b = image.size(0)
        mask_token = self.mask_token.unsqueeze(1).expand(-1, b, -1)
        ...
        if torch.all(prev_mask == torch.zeros_like(prev_mask)):
            ...
            early_feature_list, backbone_vit_feature, multi_scale_features = self.frozen_vit_backbone_forward(image, coord_features)  # frozen vit exacts early feature
            early_feature=self.gatefusion(early_feature_list[0],early_feature_list[1])
            outputs = self.click_refiner(early_feature, multi_scale_features, backbone_vit_feature, coord_features, image, prev_mask, prev_mask, mask_token)
        ...
        early_feature_list, backbone_vit_feature, multi_scale_features = self.frozen_vit_backbone_forward(image, coord_features)
        early_feature=self.gatefusion(early_feature_list[0],early_feature_list[1])
        outputs = self.click_refiner(early_feature, multi_scale_features, backbone_vit_feature, coord_features, image, prev_mask, prev_mask_modulated, mask_token)
        outputs['instances'] = nn.functional.interpolate(outputs['instances'], size=image.size()[2:], mode='bilinear', align_corners=True)
    ...
    def frozen_vit_backbone_forward(self, image, coord_features=None):
