...
from isegm.data.datasets import GrabCutDataset, BerkeleyDataset, DavisDataset, \
    SBDEvaluationDataset, PascalVocDataset, BraTSDataset, ssTEMDataset, OAIZIBDataset, HARDDataset, iSAIDDataset, \
    ISPRSPotsdamDataset, ISPRSVaihingenDataset, NWPUDataset, WHUBuildingTestDataset, SSDDDataset,LoveDADataset,SandBarDataset
...
def get_dataset(dataset_name, cfg):
    if dataset_name == 'GrabCut':
        ...
        dataset = ISPRSVaihingenDataset(cfg.VAIHINGEN_PATH, split='val_rgb')
    elif dataset_name == 'Potsdam':
        dataset = ISPRSPotsdamDataset(cfg.POTSDAM_PATH, split='val')
    elif dataset_name == 'iSAID':
        dataset = iSAIDDataset(cfg.ISAID_PATH, split='val')
    elif dataset_name == 'NWPU':
        dataset = NWPUDataset(cfg.NWPU_PATH)
    elif dataset_name == 'SSDD':
        dataset = SSDDDataset(cfg.SSDD_PATH)
    elif dataset_name == 'WHUBuilding':
        dataset = WHUBuildingTestDataset(cfg.WHUBUILDING_TEST_PATH)
    elif dataset_name == 'LoveDA':
        dataset = LoveDADataset(cfg.LOVEDA_PATH)
    elif dataset_name == 'SandBar':
        dataset = SandBarDataset(cfg.SANDBAR_PATH, split='new_val')
    else:
        dataset = None
